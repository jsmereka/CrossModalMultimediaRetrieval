	...
	double *probs, mwIndex *irad, mwIndex *jcad, int startcond)
{
	...
	for (iter=0; iter<NN; iter++) {
		...
		for (ii = 0; ii < n; ii++) {
			...
			//mexPrintf( "(1) Working on ii=%d i=%d wi=%d di=%d topic=%d author=%d phi=%d theta=%d\n" , ii , i , wi , di , topic , author , phi[wioffset+topic] , theta[aioffset+topic] );
			...
			x[i] = author;
			phi[wioffset + topic ]++; // and update counts
			phitot[topic]++;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if ((mxIsSparse( prhs[ 1 ] ) != 1) || (mxIsDouble( prhs[ 1 ] ) != 1))
		mexErrMsgTxt("THETA input matrix must be a sparse double precision matrix");
	...
	A    = mxGetM( prhs[1] );
	if (mxGetN( prhs[1] ) != D) mexErrMsgTxt("The number of documents in DS vector and AD matrix must be equal" );
	...
	*( jcphi + W ) = n;  
	...
	plhs[1] = mxCreateSparse(J, A, nzmaxtheta, mxREAL);
