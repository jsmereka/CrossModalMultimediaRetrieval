...
// Syntax
//   [ theta , mu ] = sBP( WD , phi , N , ALPHA , BETA , SEED , OUTPUT )

// Syntax
//   [ theta , mu ] = sBP( WD , phi , N , ALPHA , BETA , SEED , OUTPUT , MUIN )


void sBP( double ALPHA, double BETA, int W, int J, int D, int NN, int OUTPUT, mwIndex *jc, mwIndex *ir, double *sr,
			...
			for (i=jc[di]; i<jc[di + 1]; i++) {
				...
				for (j=0; j<J; j++) {
					mu[i*J + j] = ((double) phi[wi*J + j] + (double) BETA)/((double) phitot[j] + (double) WBETA)*
						((double) theta[di*J + j] - (double) xi*mu[i*J + j] + (double) ALPHA);
					mutot += mu[i*J + j];
				}
				for (j=0; j<J; j++) {
					mu[i*J + j] /= mutot;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
	...
	sBP(ALPHA, BETA, W, J, D, NN, OUTPUT, jc, ir, sr, phi, theta, mu, startcond);
