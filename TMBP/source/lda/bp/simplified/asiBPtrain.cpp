...
// Syntax
//   [ PHI , THETA , MU ] = asiBP( WD , J , N , ALPHA , BETA , SEED , OUTPUT )

// Syntax
//   [ PHI , THETA , MU ] = asiBP( WD , J , N , ALPHA , BETA , SEED , OUTPUT , MUIN )


void asiBP(double ALPHA, double BETA, int W, int J, int D, int NN, int OUTPUT, double *sr, mwIndex *ir, mwIndex *jc,
		...
		for (ii=0; ii<D; ii++) {
			...
			for (i=jc[di]; i<jc[di + 1]; i++) {
				...
				for (j=0; j<J; j++) {
					...
					mu[i*J + j] = ((double) phi[wi*J + j] + xi*mu[i*J + j] + (double) BETA)/
						((double) phitot[j] + xi*mu[i*J + j] + (double) WBETA)*
						((double) theta[di*J + j] + xi*mu[i*J + j] + (double) ALPHA);
					mutot += mu[i*J + j];
				}
				for (j=0; j<J; j++) {
					mu[i*J + j] /= mutot;
